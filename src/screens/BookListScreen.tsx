...
export default function BookListScreen() {
  ...
  const available = filtered.filter(b => b.stock > 0);
  const soldOut   = filtered.filter(b => b.stock === 0);
  ...
  const handleAdd = (item: Book) => {
    ...
    if (item.stock === 0) {
      ...
      Alert.alert('Stock limit reached', `You already have ${qty} in your cart.`);
    } else {
      addToCart(item);
    }
  };
          ...
          return (
            ...
            >
              <TouchableOpacity onPress={() => navigation.navigate('Book', { book: item })}>
                ...
                <Text style={styles.bookMeta}>${item.price.toFixed(2)} | Stock: {item.stock}</Text>
              </TouchableOpacity>
              <TouchableOpacity
                ...
                disabled={!canAdd}
              >
                <Text style={styles.cartButtonText}>{ item.stock === 0 ? 'Sold Out' : canAdd ? 'Add to Cart' : 'Max in Cart' }</Text>
              </TouchableOpacity>
            </View>
          );
        }}
          ...
          return (
            <TouchableOpacity key={routeName} onPress={()=>navigation.navigate(routeName)}>
              <View style={isActive?styles.activeCircle:styles.iconWrapper}>
                <Feather name={ routeName==='Home'?'home':routeName==='BookList'?'shopping-bag':'user' } size={24} color={isActive?'#fff':'#936B38'} />
              </View>
            </TouchableOpacity>
          );
        })}
...
const styles = StyleSheet.create({
  ...
  topBar:{ flexDirection:'row', alignItems:'center', paddingHorizontal:16, paddingTop:16, paddingBottom:12 },
  searchInput:{ flex:1, backgroundColor:'#E9D8C8', paddingVertical:10, paddingHorizontal:16, borderRadius:12 },
  ...
  cartButton:{ backgroundColor:'#949B7A', margin:10, paddingVertical:8, borderRadius:10, alignItems:'center' },
  cartButtonDisabled:{ backgroundColor:'#ccc' },
  cartButtonText:{ color:'#fff', fontWeight:'600' },
  bottomNav:{ position:'absolute', bottom:0, left:0, right:0, flexDirection:'row', justifyContent:'space-around', alignItems:'center', backgroundColor:'#fff', paddingTop:10, borderTopColor:'#eee', borderTopWidth:1 },
  activeCircle:{ backgroundColor:'#936B38', padding:12, borderRadius:999 },
